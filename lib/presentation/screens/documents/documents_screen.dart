...
class _DocumentsScreenState extends ConsumerState<DocumentsScreen>
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              Text(
                'Choose how to add your marksheet',
                style: TextStyle(
                  color: context.colors.textHint,
                  fontSize: 13,
                  fontFamily: 'Poppins',
              ...
              Padding(
                ...
                child: Text(
                  '📄 PDF marksheets are parsed automatically via the local OCR service',
                  style: TextStyle(
                    color: context.colors.textHint,
                    fontSize: 11,
                    fontFamily: 'Poppins',
  ...
  /// Pick & parse a PDF marksheet via PdfParserService
  Future<void> _scanFromPdf() async {
    final ocrNotifier = ref.read(ocrProvider.notifier);
    final result = await ocrNotifier.scanFromPdf();
    if (result == null) {
      // Show error if state has one
      final errMsg = ref.read(ocrProvider).errorMessage;
      if (mounted && errMsg != null) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(errMsg, style: const TextStyle(fontFamily: 'Poppins', color: Colors.white)),
    ...
    // Reuse the same downstream logic as image scan
  ...
  /// Shared post-scan logic: mismatch checks, review screen, auto-save.
  Future<void> _handleScanResult(OcrResult result) async {
