...
class _ProfileScreenState extends ConsumerState<ProfileScreen>
  ...
  bool _twelfthFromOcr = false;
  bool _gradFromOcr = false;
  bool _dobFromOcr = false;
  ...
  void _loadSavedProfile() {
    ...
    if (profile != null) {
      ...
      _twelfthFromOcr = profile.twelfthBoard.isNotEmpty && profile.isVerified;
      ...
      _gradFromOcr = profile.gradCourse.isNotEmpty && profile.isVerified;

      // DOB — locked if it came from 10th marksheet
      _dobFromOcr = profile.dateOfBirth.isNotEmpty && profile.tenthBoard.isNotEmpty && profile.isVerified;
  ...
  // ── Validation ────────────────────────────────────────────────────────────
  bool _validateAll({bool skipTwelfth = false, bool skipGrad = false, bool skipDob = false}) {
    bool valid = true;

    final nameErr = ProfileValidators.validateName(_nameCtrl.text);
    final dobErr = skipDob ? null : ProfileValidators.validateDob(
    ...
    final boardErr = (!skipTwelfth && _twelfthBoardSelection == 'Other (not listed)')
        ? ProfileValidators.validateBoardName(_customBoardCtrl.text)
        : null;
    final twelfthYearErr = skipTwelfth ? null : ProfileValidators.validateYear(
      _twelfthYearCtrl.text, minYear: 1950, maxYear: DateTime.now().year,
    );
    final twelfthPercentErr = skipTwelfth ? null : ProfileValidators.validatePercentage(_twelfthPercentCtrl.text);
    final gradYearErr = skipGrad ? null : ProfileValidators.validateYear(
      _gradYearCtrl.text, minYear: 1950, maxYear: DateTime.now().year + 4,
    );
    final gradPercentErr = skipGrad ? null : ProfileValidators.validatePercentage(_gradPercentCtrl.text);
  ...
  Future<void> _handleSave() async {
    // When OCR-locked sections exist, skip their manual validation
    if (!_validateAll(skipTwelfth: _twelfthFromOcr, skipGrad: _gradFromOcr, skipDob: _dobFromOcr)) {
    ...
    // If DOB is OCR-locked, reconstruct from individual controllers (already filled)
            ...
            children: [
              ...
              _animItem(1, _buildSection('Personal Information', Icons.person_outline_rounded, [
                ...
                if (_dobFromOcr) ...[
                  ...
                  _buildOcrChip('DOB', '${_dobDayCtrl.text}/${_dobMonthCtrl.text}/${_dobYearCtrl.text}'),
                  const SizedBox(height: 4),
                  _buildOcrSourceNote('10th marksheet'),
                  const SizedBox(height: 4),
                  _buildOcrUnlockButton(() => setState(() => _dobFromOcr = false)),
                ] else
                  _buildDobField(),
                const SizedBox(height: 12),
              ...
              _animItem(3, _buildSection('Academic Information', Icons.school_outlined, [
                ...
                // 12th — from OCR (locked) or editable
                _buildSubHeader('12th Standard', Icons.looks_two_rounded),
                const SizedBox(height: 8),
                if (_twelfthFromOcr) ...[
                  _buildOcrChip('Board', _twelfthBoardSelection ?? ''),
                  const SizedBox(height: 6),
                  _buildOcrChip('Year', _twelfthYearCtrl.text),
                  const SizedBox(height: 6),
                  _buildOcrChip('Percentage', _twelfthPercentCtrl.text),
                  const SizedBox(height: 4),
                  _buildOcrSourceNote('12th marksheet'),
                  const SizedBox(height: 6),
                  _buildOcrUnlockButton(() => setState(() => _twelfthFromOcr = false)),
                ] else ...[
                  _buildBoardPicker(),
                  const SizedBox(height: 10),
                  Row(children: [
                ...
                if (_gradFromOcr) ...[
                  ...
                  const SizedBox(height: 4),
                  _buildOcrSourceNote('graduation certificate'),
                  const SizedBox(height: 6),
                  _buildOcrUnlockButton(() => setState(() => _gradFromOcr = false)),
                ] else ...[
                  _buildCoursePicker(),
                  const SizedBox(height: 10),
  ...
  Widget _buildOcrUnlockButton(VoidCallback onUnlock) {
    return GestureDetector(
      onTap: () {
        showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            title: Text('Edit Marksheet Data?', style: TextStyle(color: context.colors.textPrimary, fontFamily: 'Poppins', fontSize: 16, fontWeight: FontWeight.w600)),
            content: Text(
              'This field was auto-filled from your scanned marksheet. Editing it manually may cause eligibility mismatches.\n\nAre you sure you want to unlock it?',
              style: TextStyle(color: context.colors.textHint, fontFamily: 'Poppins', fontSize: 13),
            ),
            actions: [
              TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Cancel', style: TextStyle(color: context.colors.textHint, fontFamily: 'Poppins'))),
              TextButton(
                onPressed: () { Navigator.pop(ctx); onUnlock(); },
                child: Text('Unlock & Edit', style: TextStyle(color: context.colors.urgencyHigh, fontFamily: 'Poppins', fontWeight: FontWeight.w600)),
              ),
            ],
          ),
        );
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: context.colors.urgencyHigh.withValues(alpha: 0.08),
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: context.colors.urgencyHigh.withValues(alpha: 0.25)),
        ),
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.lock_open_rounded, size: 12, color: context.colors.urgencyHigh),
          const SizedBox(width: 5),
          Text('Edit manually', style: TextStyle(color: context.colors.urgencyHigh, fontSize: 11, fontFamily: 'Poppins', fontWeight: FontWeight.w500)),
        ]),
      ),
    );
  }
